...
export default function SavedPage() {
  ...
  const fetchArticles = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/articles?saved=true&archived=false');
  ...
  const handleArticleUpdate = useCallback((updated: Article) => {
    setArticles(prev => {
      ...
      return mapped.filter(a => a.is_saved);
    });
  }, []);
  ...
  return (
    <div>
      <h1 className="text-xl font-semibold text-[#e2e8f0] mb-4">Saved</h1>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-[#6b7280]">
          <p className="text-lg font-medium text-[#94a3b8]">No saved articles</p>
